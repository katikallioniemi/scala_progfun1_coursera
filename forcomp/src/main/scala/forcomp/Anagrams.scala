...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
   val grouped = w.toLowerCase.toList.groupBy(c => c)
     grouped.map(x => (x._1, x._2.length)).toList.sortWith(_._1 < _._1)
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    wordOccurrences(s.reduce((a: String, b:String) => a+b))
...
//Occurences List((e,2), (h,2), (l,4), (o,3), (u,1), (y,1))
  ...
  occurrences match {
    ...
    case head :: tail => {
      ...
      (for {
        combo <- rest
        howmany <- 1 to head._2
      } yield (head._1, howmany) :: combo).toList ++ rest
    }
  }}
...
/*
  def combo2Words(combination: Occurrences): List[Word]= {
...
*/
 ...
 // def sentenceAnagrams(sentence: Sentence): List[Sentence] = ???
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = ???
    ...
    //Generate all possible combinations of occurences for sentence
    //go through each combination list by list // list ~~ word
    // see if occurence list matches with dictionary; continue
    // if all words in combination match occurences in dictionary --> sentence to list
